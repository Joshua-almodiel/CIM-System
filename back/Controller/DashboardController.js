...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const processingVaccinations = await Vaccinations.countDocuments({
      status: "Processing",
    });
    const totalChildRecords = await Parents.countDocuments();
    const totalVitalSigns = await Vitals.countDocuments();
    const totalVaccinated = await Vaccinations.countDocuments({
      status: "Scheduled",
    });
    ...
    return res
      .status(500)
      .json({ success: false, error: "Dashboard stats error" });
  }
};
